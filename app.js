...
app.get('/', async(req, res, next) => {
   return res.status(200).send({
    status: false,
    resultado: 'APIS ONLINE | BY YWHITE'
  });
  })
// ConsultasMkBuscas
...
const apiId = "22565646"; //https://my.telegram.org/auth

const apiHash = "b3eaf57a9a3953042371bb6b553cf43a"; //https://my.telegram.org/auth

const stringSession = new StringSession("")
...
(async () => {
	...
	await telegram.start({
		...
		phoneNumber: "551591401730", // SEU NUMERO DE TELEFONE AQUI DA CONTA DO TELEGRAM QUE DESEJA USAR!
...
if (!apiKey) { return res.status(401).json({error: '➭ Chave da API ausente. Coloque sua api-key, caso não tenha compre a sua já. Proprietário: @TipoWhitex'})}
...
if (!user) { return res.status(403).json({error: '➭ Chave da API inválida ou você não possui uma api-key' })}
 ...
 if (type.search(/cpf1|cpf2|cpf3|cpf4|tel1|tel2|tel3|cnpj|score|chkgg|nome|parentes|beneficios|placa1|rg|vizinhos|site|ip|cep|bin|email|cns|telefone/) === -1) return res.send('Tipo de consulta invalida');
			...
			try {
               ...
               "str": "[⚠️] INVALIDO! [⚠️]"
				...
				if ((chatId == i.chat && senderId == i.bot) && (msgggveri.includes(queryverii) || txtuuuveri.includes(queryverii) )) {
                    ...
                    str = str.replace(/BY: @MkBuscasRobot/gi, "©️ BY: @Distinction_Search_bot");
	...
	setTimeout(() => {
		...
		res.json({
               ...
               "resultado": {
               "str": "☝️🤓 | servidor demorou muito para responder"
               }
             })
	}, 10000);
});
